...
const PaymentScreen = ({pickupInfo, isVisible, onCancelRequest, onClose}: IProps) => {
  const {
  ...
  return (
    <Modal visible={isVisible} onRequestClose={onClose} transparent statusBarTranslucent>
      <ScreenWrapper barColor="white" barStyle="dark-content" translucentBar>
        <View h={hp(100) + STATUSBAR_HEIGHT} w="full">
          <MapSection
            height={hp(100)}
            coordinates={[
              {latitude: parseFloat(destinations[0].lat), longitude: parseFloat(destinations[0].long)},
        ...
        <View px="10px" pt="10px" bg="bg" position="absolute" bottom="0" borderTopRadius="2xl" w="full" h={hp(50)}>
          <ScrollView bounces={false}>
            ...
            <PaymentMethodSection method={paymentChannel} onChangeCard={setSelectedCardId} selectedCardId={selectedCardId} />
            ...
            {/* bottom button */}
            <HStack w="full" bottom="0" alignItems="center" justifyContent="space-between" mb="10px" mt="5%" px="10px">
              <Button bg="black" title="Cancel Pickup" isLoading={isCancelling} w="48%" onPress={handleCancelPickup} />
