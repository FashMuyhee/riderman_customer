...
const PaymentScreen = ({pickupInfo, isVisible, onCancelRequest, onClose}: IProps) => {
  ...
  const {delivery_locations, paymentChannel, pickupLocation, rider, totalAmount, delivery_packages, pickupRequestId} = pickupInfo;
  ...
  const handlePayment = async () => {
    setIsLoading(true);
    try {
      const res = await deliveryService.makeCashPayment(pickupRequestId.toString());
      setIsLoading(false);
      if (res?.success) {
        setIsPaid(true);
      }
